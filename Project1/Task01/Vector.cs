...
namespace DataStructures_Algorithms.Project1
{
    ...
    public class Vector<T> : ISorter
    {
        ...
        {
            IComparer<T> comparer = Comparer<T>.Default;
            T pivot_element = data[left];
            int lb = left, ub = right;
            T temp;

            while (left < right)
            {
                if (comparer.Compare(data[left], pivot_element) <= 0)
                    left++;
                if (comparer.Compare(data[right], pivot_element) > 0)
                    right--;
                if (left < right)
                {
                    temp = data[left];
        ...
        void MergeSort()
        {
            int left = 0;
            int right = Count - 1;
            InternalMergeSort(data, left, right);
        }

        private void InternalMergeSort(T[] inputArray, int left, int right)
        {
            int mid = 0;
            if (left < right)
            {
        ...
        private void MergeSortedArray(T[] inputArray, int left, int mid, int right)
        {
            IComparer<T> comparer = Comparer<T>.Default;
            int index = 0;
            ...
            {
                if (comparer.Compare(inputArray[left], inputArray[right_start]) < 0)
                {
